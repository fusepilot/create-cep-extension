...
function cleanup {
  echo 'Cleaning up.'
  ps -ef | grep 'create-cep-extension-scripts' | grep -v grep | awk '{print $2}' | xargs kill -s 9
  cd "$root_path"
...
cd "$root_path"/packages/create-cep-extension-scripts
...
cd $temp_app_path
create_react_app --scripts-version="$scripts_path" --internal-testing-template="$root_path"/packages/create-cep-extension-scripts/fixtures/kitchensink test-kitchensink
...
npm link "$root_path"/packages/create-cep-extension-scripts
