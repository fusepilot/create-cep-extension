...
const program = new commander.Command(packageJson.name)
  ...
  .option(
    ...
    'use a non-standard version of create-cep-extension-scripts'
  )
  .allowUnknownOption()
  .on('--help', () => {
    ...
    console.log(
      `      - a custom fork published on npm: ${chalk.green('my-create-cep-extension-scripts')}`
    )
    console.log(
      `      - a .tgz archive: ${chalk.green('https://mysite.com/my-create-cep-extension-scripts-0.8.2.tgz')}`
    )
    console.log(`    It is not needed unless you specifically want to use a fork.`)
    console.log()
    console.log(`    If you have any problems, do not hesitate to file an issue:`)
    console.log(
      `      ${chalk.cyan('https://github.com/fusepilot/create-cep-extension/issues/new')}`
    )
    console.log()
  })
  .parse(process.argv)
...
function run (root, appName, version, verbose, originalDirectory, template) {
  ...
  getPackageName(packageToInstall)
    ...
    .then(packageName => {
      ...
      // Since create-cep-extension-scripts has been installed with --save
...
function getInstallPackage (version) {
  let packageToInstall = 'create-cep-extension-scripts'
...
function getPackageName (installPackage) {
  if (installPackage.indexOf('.tgz') > -1) {
    return getTemporaryDirectory()
      ...
      .catch(err => {
        // The package name could be with or without semver version, e.g. create-cep-extension-scripts-0.2.0-alpha.1.tgz
    ...
    // git+https://github.com/mycompany/create-cep-extension-scripts.git
    // git+ssh://github.com/mycompany/create-cep-extension-scripts.git#v1.2.3
...
function checkAppName (appName) {
  ...
  const devDependencies = ['create-cep-extension-scripts']
...
// https://github.com/fusepilot/create-cep-extension/pull/368#issuecomment-243446094
