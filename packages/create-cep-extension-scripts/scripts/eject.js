...
prompt('Are you sure you want to eject? This action is permanent.', false).then(shouldEject => {
  if (!shouldEject) {
  ...
  const folders = ['config', 'config/jest', 'scripts', 'scripts/utils', 'scripts/templates']
  ...
  appPackage.jest = createJestConfig(filePath => path.posix.join('<rootDir>', filePath), null, true)
  ...
  if (ownPath.indexOf(appPath) === 0) {
    try {
      // remove create-cep-extension-scripts and create-cep-extension-scripts binaries from app node_modules
      Object.keys(ownPackage.bin).forEach(binKey => {
