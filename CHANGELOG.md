...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.9.5
```
...
#### :nail_care: Enhancement
* `eslint-config-react-app`, `create-cep-extension-scripts`
...
* `react-dev-utils`, `create-cep-extension-scripts`
...
#### :house: Internal
* `create-cep-extension-scripts`
  * [#1736](https://github.com/facebookincubator/create-react-app/pull/1736) Fix eject for linked create-cep-extension-scripts. ([@tuchk4](https://github.com/tuchk4))
...
* `eslint-config-react-app`, `react-dev-utils`, `create-cep-extension-scripts`
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.9.4
```
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.9.3
```
...
* `create-react-app`
  * [#1253](https://github.com/facebookincubator/create-react-app/pull/1253) **Install time optimization.** ([@n3tr](https://github.com/n3tr))
    ...
    React, ReactDOM, and `create-cep-extension-scripts` are now installed in the same install instead of two different installs. This reduces app creation time by a noticeable amount.
...
* `create-cep-extension-scripts`
  ...
  * [#1356](https://github.com/facebookincubator/create-react-app/pull/1356) Fix workflow if create-cep-extension-scripts package is linked via npm-link. ([@tuchk4](https://github.com/tuchk4))

    Advanced users may opt to fork `create-cep-extension-scripts` instead of ejecting so they still receive upstream updates.<br>
    `create-cep-extension-scripts` will now function as expected when linking to a development version.<br>
...
* User Guides
  ...
  * [#1495](https://github.com/facebookincubator/create-react-app/pull/1495) Add useful link to create-cep-extension-scripts. ([@pd4d10](https://github.com/pd4d10))
* READMEs
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.9.2
```
...
* `create-cep-extension-scripts`
  ...
  * [#1489](https://github.com/facebookincubator/create-react-app/pull/1489) Support setting `"homepage"` to `"."` to generate relative asset paths. ([@tibdex](https://github.com/tibdex))
    ...
    Applications that don’t use the HTML5 `pushState` API can now be built to be served from any relative URL. To enable this, specify `"."` as your `homepage` setting in `package.json`. It used to be possible before with a few known bugs, but they should be fixed now. See [Serving the Same Build from Different Paths](https://github.com/facebookincubator/create-react-app/blob/master/packages/create-cep-extension-scripts/template/README.md#serving-the-same-build-from-different-paths).
  ...
  * [#937](https://github.com/facebookincubator/create-react-app/pull/1504) Add `PUBLIC_URL` environment variable for advanced use. ([@EnoahNetzach](https://github.com/EnoahNetzach))
    ...
    If you use a CDN to serve the app, you can now specify `PUBLIC_URL` environment variable to override the base URL (including the hostname) for resources referenced from the built code. This new variable is mentioned in the new [Advanced Configuration](https://github.com/facebookincubator/create-react-app/blob/master/packages/create-cep-extension-scripts/template/README.md#advanced-configuration) section.
  ...
  * [#1440](https://github.com/facebookincubator/create-react-app/pull/1440) Make all `REACT_APP_*` environment variables accessible in `index.html`. ([@jihchi](https://github.com/jihchi))
    ...
    This makes all environment variables previously available in JS, also available in the HTML file, for example `%REACT_APP_MY_VARIABLE%`. See [Referencing Environment Variables in HTML](https://github.com/facebookincubator/create-react-app/blob/master/packages/create-cep-extension-scripts/template/README.md#referencing-environment-variables-in-the-html).
...
* `create-cep-extension-scripts`, `react-dev-utils`
...
* `create-cep-extension-scripts`
  ...
  * [#1522](https://github.com/facebookincubator/create-react-app/pull/1522) Upgrade dependencies. ([@Timer](https://github.com/Timer))
  * [#1458](https://github.com/facebookincubator/create-react-app/pull/1458) Additionally remove `create-cep-extension-scripts` from dependencies on eject. ([@creynders](https://github.com/creynders))
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.9.0
```
...
#### :bug: Bug Fix
* `create-react-app`, `create-cep-extension-scripts`
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.8.5
```
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.8.4
```
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.8.3
```
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.8.2
```
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.8.1
```
...
* `create-cep-extension-scripts`
  * [#944](https://github.com/facebookincubator/create-react-app/pull/944) Crash the build during CI whenever linter warnings are encountered. ([@excitement-engineer](https://github.com/excitement-engineer))
    ...
    Linter warnings and errors are now checked during a continuous integration build (set by the `CI` environment variable) and the build will fail if any issues are found. See [Continuous Integration](https://github.com/facebookincubator/create-react-app/blob/master/packages/create-cep-extension-scripts/template/README.md#continuous-integration) for more information.
...
* `create-react-app`, `create-cep-extension-scripts`
  * [#898](https://github.com/facebookincubator/create-react-app/pull/898) Support Yarn. ([@fson](https://github.com/fson))
    ...
    `create-cep-extension-scripts` now also displays instructions using `yarn` commands for projects using Yarn (projects having a `yarn.lock` file).
...
* `babel-preset-react-app`
  * [#902](https://github.com/facebookincubator/create-react-app/pull/902) Enable useBuiltIns option on object-rest-spread. ([@existentialism](https://github.com/existentialism))
    ...
    **Note:** `create-cep-extension-scripts` already adds this polyfill, so no changes are necessary in Create React App projects.
...
* `react-dev-utils`, `create-cep-extension-scripts`
  * [#816](https://github.com/facebookincubator/create-react-app/pull/816) add logging of existing default port process on start. ([@ianmcnally](https://github.com/ianmcnally))
    ...
    `create-cep-extension-scripts` can guess which process is running on the port 3000 when it's not available:
    ```
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.8.0
```
...
### Build Dependency (`create-cep-extension-scripts`)
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.7.0
```
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.6.1
```
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.6.0
```

**Note: If the project fails to start, remove `node_modules`, ensure `create-cep-extension-scripts` is `0.6.0` in your `package.json`, and run `npm install` again. There seems to be an [npm bug](https://github.com/npm/npm/issues/14073) affecting this update.**
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.5.1
```
...
* Adds [support for `public` folder](https://github.com/facebookincubator/create-react-app/blob/master/packages/create-cep-extension-scripts/template/README.md#using-the-public-folder) with arbitrary assets. ([@gaearon](https://github.com/gaearon) in [#703](https://github.com/facebookincubator/create-react-app/pull/703))
* You can now [specify defaults](https://github.com/facebookincubator/create-react-app/blob/master/packages/create-cep-extension-scripts/template/README.md#adding-development-environment-variables-in-env) for environment variables with `.env` file. ([@ayrton](https://github.com/ayrton) in [#695](https://github.com/facebookincubator/create-react-app/pull/695))  
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.5.0
```
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.4.3
```
...
* Passing a custom fork of `create-cep-extension-scripts` to `create-react-app` with `--scripts-version` works again. ([@yesmeck](https://github.com/yesmeck) in [#632](https://github.com/facebookincubator/create-react-app/pull/632))
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.4.2
```
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.4.1
```
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.4.0
```
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.3.1
```
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.3.0
```
...
Update `create-cep-extension-scripts` to point to `0.2.3` in your `package.json` and run `npm install`. You shouldn’t need to do anything else.
...
Update `create-cep-extension-scripts` to point to `0.2.2` in your `package.json` and run `npm install`. You shouldn’t need to do anything else.
...
* Issues with `npm link`ing `create-cep-extension-scripts` have been fixed ([@dallonf](https://github.com/dallonf) in [#277](https://github.com/facebookincubator/create-react-app/pull/277))
...
Update `create-cep-extension-scripts` to point to `0.2.1` in your `package.json` and run `npm install`. You shouldn’t need to do anything else. If you see a warning about wrong file casing next time you `npm start`, fix your imports to use the correct filename casing.
...
```
npm install --save-dev --save-exact create-cep-extension-scripts@0.2.0
```
